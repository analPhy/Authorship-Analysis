...
def authorship_analysis():
    ...
    try:
        # Using user's original fetch_wikipedia_text_for_authorship
        ...
        if len(set(all_labels)) < 2: return jsonify({"error": "Could not gather sentences from both sources for comparison."}), 400
