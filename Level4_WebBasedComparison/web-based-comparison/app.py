...
import nltk
from nltk.tokenize import word_tokenize, sent_tokenize # sent_tokenize もインポート
...
def preprocess_tokens(tokens_list: list[str]) -> list[str]:
    ...
    if not isinstance(tokens_list, list):
        ...
        return []
    # Ensure all items are strings before processing, as NLTK might return non-strings in some edge cases.
...
REQUIRED_NLTK_RESOURCES = {
    ...
    "words": "corpora/words"
    # "punkt_tab" was removed. If specific issues arise with punkt, it might be revisited,
    # but standard punkt should be preferred.
}
...
for download_key, path_to_find in REQUIRED_NLTK_RESOURCES.items():
    try:
        ...
        try:
            ...
            # nltk.download can return True or None on success
            ...
            else: # pragma: no cover (hard to test this branch reliably if download succeeds/fails with exception)
...
# --- Flask App Setup ---
app = Flask(__name__)
# Using user's latest CORS config
...
# === Text processing functions (User's existing versions are kept for now) ===
def get_text_from_url_for_kwic(url):
    ...
    try:
        ...
        for tag_name in ['script', 'style', 'sup', 'table', 'head', 'link', 'meta', 'noscript',
            ...
            for tag in soup.find_all(tag_name): # Ensure all instances are found and decomposed
                tag.decompose()
...
def tokenize_mixed_for_authorship(text: str) -> list[str]:
    initial_tokens = []
    # User's existing lang_detect and fugashi logic
    if not _TAGGER:
        logging.warning("Fugashi Tagger not available, using NLTK word_tokenize for authorship.")
        try: initial_tokens = word_tokenize(text) # Removed language='english'
...
def kwic_search():
    ...
    try:
        # Using user's original get_text_from_url_for_kwic and clean_text_for_kwic
        ...
        initial_tokens_from_page = word_tokenize(text_cleaned) # Removed language='english'
    ...
    if search_type == 'token':
        ...
        for i in range(len(words_from_page_lower) - num_target_tokens + 1):
            if words_from_page_lower[i : i + num_target_tokens] == target_token_list_lower:
                ...
                results.append({
                    ...
                    "original_text_index": i # For user's custom sort
                })
        ...
        try:
            tagged_words = nltk.pos_tag(words_from_page_processed) # Use processed tokens, no lang_eng issue expected
        ...
        try:
            tagged_words_for_ner = nltk.pos_tag(words_from_page_processed) # POS tagging with processed tokens
    ...
    if results:
        # logging.info(f"Applying custom sorting to {len(results)} results.") # Already logged by user
        pattern_groups = {}
        ...
        if freq_and_pos_groups:
            ...
            results = sorted_results_final # Overwrite with sorted results
            # logging.info(f"Custom sorting applied. Result count: {len(results)}") # User's log has similar
        else:
            logging.info("No patterns for custom sorting or all items were at end of context. Defaulting to original find order (if available).")
            if all("original_text_index" in r for r in results): # Ensure key exists
...
# === Authorship Analysis Endpoint (User's version with updated tokenizer) ===
...
def authorship_analysis():
    ...
    if not url_a or not url_b: return jsonify({"error": "Please provide two Wikipedia URLs."}), 400 # User's original message
    
    for i, url_val in enumerate([url_a, url_b]): # User's original validation logic
    ...
    try:
        ...
        MIN_SAMPLES_PER_CLASS_FOR_STRATIFY = 2 # User's constant
        ...
        for idx, author_label_from_model in enumerate(clf.classes_): # Using actual class label from model
